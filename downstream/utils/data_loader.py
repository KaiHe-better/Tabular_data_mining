...
class RandomShiftTable(Dataset):
    ...
    def __init__(self, tokenizer, args, has_header=True, label_column_index=None):
        ...
        self.col_name = list(self.dataframe.head())
    ...
    def __getitem__(self, index):
        ...
        input_len_list = []
        for x in X:
            tmp_x = self.tokenizer.encode(x, add_special_tokens=False, return_tensors="pt").squeeze(0)[:self.max_cell_len-1]
...
def get_loader(args, tokenizer, num_workers=multiprocessing.cpu_count()):
    ...
    return data_loader, args, dataset.col_name


class AccessTable(Dataset):
    """docstring for AccessTable"""
    def __init__(self, tokenizer, args, idx, has_header=True):

        with open(args.train_file_path, 'r', encoding='utf-8') as csv_file:
            self.dataframe = pd.read_csv(csv_file, header=0 if has_header else None, dtype=str)
        self.dataframe = self.dataframe.iloc[idx]
        self.dataframe.fillna(" ", inplace=True)
        ...
        # 删标签的列数
        self.num_columns = self.dataframe.shape[1]-1
    ...
    def __getitem__(self, index):
        y = int(self.dataframe.iloc[index,-1])
        ...
        return {'sent': torch.stack(X), 'y_label': torch.tensor([y]).long(), 'input_len_list':input_len_list}

def shift_collate_fn2(data):
    batch_data = {'sent': [], 'y_label': [], 'input_len_list': []}
    for data_item in data:
        for k, v in batch_data.items():
            tmp = data_item[k]
            batch_data[k].append(tmp)
    ...
    batch_data['sent'] = torch.stack(batch_data['sent'])   
    batch_data['y_label'] = torch.stack(batch_data['y_label']).squeeze()
    batch_data['input_len_list'] = torch.tensor(batch_data['input_len_list'])
    return batch_data


def get_loader2(args, tokenizer, idx, num_workers=multiprocessing.cpu_count()):
    dataset = AccessTable(tokenizer, args, idx, has_header=True)
    data_loader = DataLoader(dataset=dataset,
                                    batch_size=args.batch_size,
                                    shuffle=True,
                                    pin_memory=True,
                                    num_workers=num_workers,
                                    collate_fn=shift_collate_fn2,
                                 )
    args.num_columns = dataset.num_columns                            
    return data_loader, args, dataset.col_name