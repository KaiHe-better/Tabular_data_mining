...
class My_Trainer:
    ...
    def __init__(self, args, train_data_loader, col_name, tokenizer, device):
        self.args = args
        self.device = device
    ...
    def retrieval(self, my_model, train_data_loader, test_data_loader):
        with torch.no_grad(): 
            ...
            cnt = 0 
            tbar_train = tqdm(train_data_loader, total=len(train_data_loader), disable=False, desc="retrieval_train", ncols=150)
            data_logreg_train = {'sent':[],'y_label':[]}
            for data_item in tbar_train:
                if torch.cuda.is_available():
                    data_item['sent'] = data_item['sent'].to(self.device)
                    # data_item["y_label"] = data_item["y_label"].to(self.device)
                # if cnt==0:
                col_embed = self.get_col_name_index(data_item["sent"])
                cnt+=1

                data_emb_batch = self.my_model.get_emb(data_item, col_embed).cpu()
                #print (data_emb_batch.shape)
                #print (data_item['y_label'].shape)
                data_logreg_train['sent'].append(data_emb_batch)
                data_logreg_train['y_label'].append(data_item['y_label'])
            data_logreg_train['sent'] = torch.vstack(data_logreg_train['sent']) 
            data_logreg_train['y_label'] = torch.hstack(data_logreg_train['y_label'])


            tbar = tqdm(test_data_loader, total=len(test_data_loader), disable=False, desc="retrieval", ncols=50)
            data_logreg_test = {'sent':[],'y_label':[]}
            for data_item in tbar:
                ...
                #print (data_emb_batch.shape)
                #print (data_item['y_label'].shape)
                data_logreg_test['sent'].append(data_emb_batch)
                data_logreg_test['y_label'].append(data_item['y_label'])
            data_logreg_test['sent'] = torch.vstack(data_logreg_test['sent']) 
            data_logreg_test['y_label'] = torch.hstack(data_logreg_test['y_label'])


        return data_logreg_train, data_logreg_test
    
    def get_col_name_index(self, sent):
        tmp_X_list = []
        with torch.no_grad():
            for x in self.col_name:
                tx = torch.tensor(self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(x))).to(self.device)
                tmp_X_list.append(torch.mean(self.my_model.sentence_encoder.embeddings.word_embeddings(tx), dim=0) )
        col_emb = torch.stack(tmp_X_list).unsqueeze(0).unsqueeze(2).repeat(sent.size(0), 1, sent.size(2), 1)
        tmp_size = list(col_emb.size())
        tmp_size[1] = 1
        _pad = torch.zeros(tmp_size).to(self.device)
        col_emb = torch.cat( (_pad, col_emb, _pad), dim=1)
        return col_emb
    ...
    def train(self, my_model):
        ...
        for epoch_iter in range(self.args.epoch):
            ...
            tbar = tqdm(self.train_data_loader, total=len(self.train_data_loader), disable=False, desc="Training", ncols=100)
            for data_item in tbar:
                if torch.cuda.is_available():
                    data_item["sent"] = data_item["sent"].to(self.device)
